...
while True:
    ...
    if c == "1":  # SSS
        ...
        area = sqrt(
            semiSum * (semiSum - sides[0]) * (semiSum - sides[1]) * (semiSum - sides[2])
        )  # Apparently Heron's Formula, thanks GitHub Copilot
        circleRadius = (sides[0] * sides[1] * sides[2]) / (
            4 * area
        )  # Radius of the circumscribed circle. Thanks again, Copilot

        angles[0] = (180 / pi) * asin(
            (sides[0] / (circleRadius * 2))
        )  # Calculate corners using sides
        angles[1] = (180 / pi) * asin(
            (sides[1] / (circleRadius * 2))
        ...
        angles[1] = degrees(
            asin((sides[1] * sin(radians(angles[0]))) / sides[0])
        )  # I was stuck here for SO LONG because I forgot to do DEGREES()
        ...
        sides[2] = (sin(radians(angles[2])) * sides[0]) / sin(
            radians(angles[0])
        )  # Thanks mathsisfun.com
        ...
        sides[2] = sqrt(
            sides[0] ** 2
            + sides[1] ** 2
            - 2 * sides[0] * sides[1] * cos(angles[2] * pi / 180)
        )  # Thanks Copilot

        semiSum = (
            sides[0] + sides[1] + sides[2]
        ) / 2  # Code hasn't finished DRYing yet
        area = sqrt(
            semiSum * (semiSum - sides[0]) * (semiSum - sides[1]) * (semiSum - sides[2])
        )
        ...
        sides[0] = (sides[2] * sin(radians(angles[0]))) / sin(
            radians(angles[2])
        )  # Thanks Copilot
